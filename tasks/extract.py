...
class ExtractTask(Task):
    ...
    def standard_prompt_wrap(x:dict, y:str='') -> str:
        extract_config = yaml.safe_load(open('prompts/extract.yaml', 'r'))
        standard_prompt = extract_config['extract']['prompt']
        return standard_prompt.format(text=x['text'], keyword=x['keyword']) + y
    ...
    def eval_prompt_wrap(x, y_pred: str, ideal: str, type='fact') -> str:
        ...
        eval_prompt = grader_config['fact']['prompt'].format(input=x['text'], ideal=ideal, completion=y_pred) 
        return eval_prompt 